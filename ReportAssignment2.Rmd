...
```{r}
glm2 <- glm(Survived ~ Age * PClass, data=data_titanic, family="binomial")
anova(glm2, test="Chisq")
```

We observe that the interaction Age-PClass is not significant, since it has a p-value of 0.558\>0.05.

```{r}
glm3 <- glm(Survived ~ Age * Sex, data=data_titanic, family="binomial")
anova(glm3, test="Chisq")
```

In this case, we see that the interaction Age-Sex is significant(p-value=5.645e-07 \< 0.05), so we have to include it in our model. PClass is also significant by itself so we have to include it too.

So the final model is:

```{r}
final_model <- glm(Survived ~ PClass + Age+ Sex + Age*Sex, data=data_titanic, family="binomial")
summary(final_model, test="Chisq")
```

From the table above, we observe that Sexmale and **Age** are not statistically significant. This model has an AIC of 679.1, which is lower than the additive model’sAIC of 705.1. Since a lower AIC indicates a better balance between fit and complexity, we choose this model over the additive one.
...
```{r}
new_data <- expand.grid(Age = 55, 
                        PClass = levels(data_titanic$PClass), 
...
```{r}
chisq.test(cont_table_class)
```

Both tests show that class and sex are strongly associated with survival.

The 2-test for the relationship between passenger class (PClass) and **survival status (Survived)** reveals a significant association, with a test statistic of 172 and a p-value less than 2e-16. This indicates that survival is strongly influenced by passenger class, with the null hypothesis of no association being rejected. Similarly, Fisher’s Exact Test for **gender (Sex)** and **survival status (Survived)** shows a highly significant result (p-value \< 2e-16). The odds ratio of 0.1 suggests that females have much higher odds of surviving than males, with a 95% confidence interval (0.0762, 0.1316) confirming that the true odds ratio is significantly less than 1. This supports the conclusion that being female substantially increases the chances of survival.
